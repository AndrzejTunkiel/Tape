...
def run_me():
    ...
    x1 = Bidirectional(GRU(units=32, kernel_initializer =  'glorot_uniform',
               ...
               return_state=False, stateful=False))(visible1)
    ...
    drop2 = Dropout(0.1)(pool11)
    dense2 = Dense(hDense2)(drop2)
    flat1 = Flatten()(drop2)

    dense2 = Dense(hDense3)(visible1)
    flat2 = Flatten()(dense2)
    
    combined = concatenate([x1, flat1, flat2])
    
    z = Dense(imagination, activation="relu")(combined)
    ...
    history = model.fit(X_train_m_a,y_train_RNN_a,validation_data=(X_train_m_b,y_train_RNN_b),
                                    epochs=2000, verbose=1, batch_size=4,
                                    callbacks=[es, mc])
    
    model = load_model('best_model.h5')
    

    result_test = model.evaluate(X_test_m, y_test_RNN)
...
# print(f'Mean: {np.mean(results, axis=0)}')
