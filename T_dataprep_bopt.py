...
def optimize_me(memory_meters, step_extension, min_required_variance,GRUunits,
    ...
    if PCA_n == -1:
        ...
        X_train_MLP = np.concatenate(X_train_MLP, axis = 1)
        ...
        X_test_MLP = np.concatenate(X_test_MLP, axis = 1)
    ...
    X_train_m = [X_train_RNN_m, X_train_MLP_m]
    
    X_test_RNN_m = X_test_RNN[:,:,np.newaxis]
    X_test_MLP_m = X_test_MLP[:,:,np.newaxis]
    X_test_m = [X_test_RNN_m, X_test_MLP_m]
    #%%
    ...
    input2 = Input(shape=((memory+imagination)*len(X_train[0]),1))
    
    
    
    x1 = GaussianNoise(0.02)(input1)
    ...
    x2 = Dense(dense_units_2, kernel_initializer = 'glorot_uniform')(x2)
    ...
    history = model.fit(X_train_m,y_train_RNN,validation_data=(X_test_m,y_test_RNN),
                                epochs=2000, verbose=0, batch_size=8,
                                callbacks=[es, mc])

    return -np.log10(model.evaluate(X_test_m, y_test_RNN, verbose=0))
#%%
...
pbounds = {'memory_meters': (1, 50), 
           'step_extension': (5, 20), 
           'min_required_variance': (0.7, 0.99),
           'GRUunits': (2, 2048),
           'dense_units_1': (2, 256),
           'dense_units_2': (2, 256)}
