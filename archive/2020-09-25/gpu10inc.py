...
from training_delta_ensemble import train_delta as train_delta_e
from training_delta_best import train_delta as train_delta_b
...
try:
    aaes_ticket = np.load('ticket_cont_results.npy', allow_pickle=True)
    aaes_ave = np.load('ave_cont_results.npy', allow_pickle=True)
    r2_ticket = np.load('ticket_cont_results_r2.npy', allow_pickle=True)
    r2_ave = np.load('ave_cont_results_r2.npy', allow_pickle=True)
    print('Success loading old results', allow_pickle=True)
except:
...
while True:
    ...
    for i, val in enumerate(hypers):
        ...
        val_loss, test_loss, aae,ypred, ytrue = train_delta_b(val)
    ...
    np.save('ticket_cont_results.npy', aaes_ticket)
    np.save('ticket_cont_results_r2.npy', r2_ticket)
    ...
    for i, val in enumerate(hypers):
        ...
        val_loss, test_loss, aae,ypred, ytrue = train_delta_e(val)
        aae_array.append(aae)  
        r2_array.append(r2_score(ypred, ytrue))
    ...
    np.save('ave_cont_results.npy', aaes_ave)
    np.save('ave_cont_results_r2.npy', r2_ave)
